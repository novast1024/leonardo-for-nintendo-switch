...
namespace hidVM {
    ...
    bool update(uint8_t *data) {
        ...
        if ((opcode & 0x80) == 0) {
            switch (opcode) {
                default: return false; // NOT DEFINED CODE
                case 0x0e: // LoopStart
            ...
            if (opcode & 0x01) { data[0] = *pos++; } else { data[0] = 0x00; }
            if (opcode & 0x02) { data[1] = *pos++; } else { data[1] = 0x00; }
            if (opcode & 0x04) { data[2] = *pos++; } else { data[2] = 0x00; }
            if (opcode & 0x08) { data[3] = *pos++; } else { data[3] = 0x80; }
            if (opcode & 0x10) { data[4] = *pos++; } else { data[4] = 0x80; }
            if (opcode & 0x20) { data[5] = *pos++; } else { data[5] = 0x80; }
            if (opcode & 0x40) { data[6] = *pos++; } else { data[6] = 0x80; }
            pos = decode_varint_u32(pos, end, hold_time);
            hold_start = millis();
            return true;
        }
