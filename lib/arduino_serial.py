...
from model.gamepad import Gamepad
...
LOOP_START_OPCODE = b"\x01"
LOOP_END_OPCODE = b"\x02"
...
def encode_instruction(data: HoldData) -> bytes:
    ...
    opcode = basic_report_to_opcode.get(data.gamepad.to_hid_report(), None)
    if opcode is not None:
        return bytes(itertools.chain(opcode, bytes(varint.encode(ms))))
    else:
        opcode = 0x80
        diff = bytearray()
        for i, byte in enumerate(data.gamepad.to_hid_report()):
