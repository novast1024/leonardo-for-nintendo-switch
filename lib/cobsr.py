def encode(data: bytes) -> bytes:
    ...
    buffer = bytearray(len(data) + len(data) // 254 + 1)
    ...
    for byte in data:
        if byte != 0:
            ...
            code += 1
            if code == 0xFF:
                # Finalize the current code and start a new block
    ...
    if data[-1] >= code:
        buffer[code_index] = data[-1]
        write_index -= 1
    else:
        buffer[code_index] = code

    return bytes(buffer[:write_index])
...
def decode(data: bytes) -> bytes:
    ...
    while True:
        code = data[i]
        if (code > len(data) - i):
            # The final block with 1-byte reduction (reduced case)
            result.extend(data[i+1:len(data)])
            result.append(code)
            break
        else:
            ...
            i += code
            if i == len(data):
                # The current block is final (not-reduced case)
                break
            if code != 0xFF:
