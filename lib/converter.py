...
def encode(data: bytes, seconds: float):
    ms = round(seconds * 1000)
    code = code_map.get(data)
    if code is not None:
        if ms == 0:
            f = 0
            intsize = 0
        ...
        return int.to_bytes(code | f << 5) + int.to_bytes(ms, intsize, "little")
    else:
        code = 0x80
        for i in range(7):
            if data[i] == b"\x00\x00\x08\x80\x80\x80\x80"[i]:
                code |= 1 << i
        return bytes([code] + encode_varint(ms))

LOOP_START_CODE = 0x1c
LOOP_END_CODE = 0x1e
